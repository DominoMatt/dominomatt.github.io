...
description: "Catch Fish to collect Trophys before other players can."
...
- Turn all 28 dominoes face-up, flip the tiles w/ blank ends face-down...
  shuffling the blanks.
- Then put all the tiles (both face-up and down) into the 'Pond'.
- Place the 1(Ace)-6 (or homemade 1-6) out in a row near the fish pond as
  Trophys.
...
- If you collect 3 Fish that have an end matching the value of a given Public
  Objective (and it is still	available), you may claim it.
- When claiming the Objective take it and place all three Fish atop it, those 3
  Fish can't be used to claim any more Public Objectives.
...
- Before scoring, starting with the 1st player, players can collect Public
  Objective from any still available if they have a legal set to collect it.
  Continue in turn order until all players are done collecting.

## Scoring

Most points wins. Tie goes to the player with the fewest total fish.

- Score 1 point for each Fish you have caught.
- Score 1 point for each Trophy you collected.
