...
Dominoes are played end to end, matching an open end in the layout, progressing the line of play. 
...
- **Domino Set/Hand size** — More players... larger domino set. More players... smaller hand size. 
- **Setting the first domino** —  (Coming Soon)
- **Layout shape** — arm, line, cross, or tree. Shape is determined by how many doubles (if any) act as *spinners* (tiles that branch the layout).
- **Connection rule** — most games use Equal End Matching; a few (e.g., matador) use different matching logic.
- **Draw mode** — in a *block* game, only dealt tiles are played, and a player without a legal move passes. In a *draw* game, a player without a legal move draws from the boneyard (face-down undealt tiles) until they can play.
- **Scoring mode** — see Blocking vs. Scoring Games, below.
...
- In larger sets Layouts move toward tree layouts... where all doubles are spinners.
